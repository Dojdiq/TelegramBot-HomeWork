import telebot
from config import TOKEN, keys
from extensions import ExchangeException, Exchange

bot = telebot.TeleBot(TOKEN)  # BitAlBorBot


@bot.message_handler(commands=["start", "help"])
def start_help(message: telebot.types.Message):
    text = "To start enter command to the bot in format: \n \n<currency> \
    <what currency you like to exchange> \
    <amount of currency>\n \n List of all available currencies: /values"
    bot.reply_to(message, text)


@bot.message_handler(commands=["values"])
def values(message: telebot.types.Message):
    text = "Available currencies:"
    for key in keys.keys():
        text = '\n'.join((text, key, ))
    bot.reply_to(message, text)


@bot.message_handler(content_types=["text"])
def get_price(message: telebot.types.Message):
    try:
        values = message.text.split(' ')

        if len(values) != 3:
            raise ExchangeException("Input only 3 parameters.")

        quote, base, amount = values
        total_base = Exchange.get_price(base, quote, amount)

        text = f"Exchange {amount} {quote} in {base} - {total_base}"
        bot.send_message(message.chat.id, text)

    except ExchangeException as e:
        bot.reply_to(message, f"User error.\n{e}")
    except Exception as e:
        bot.reply_to(message, f"User error.\n{e}")


bot.polling()